...
public class MeshAssetGenerator : MonoBehaviour {
    ...
    [ContextMenu("GenerateSphereMeshAsset")]
    public void GenerateSphereMeshAsset() {
        MeshFilter mf = GetComponent<MeshFilter>();
        using (StreamWriter streamWriter = new StreamWriter(string.Format("{0}/{1}.obj", Application.streamingAssetsPath, this.gameObject.name))) {
            streamWriter.Write(SphereMeshToString(mf, new Vector3(-1f, 1f, 1f)));
            streamWriter.Close();
        }
#if UNITY_EDITOR
    ...
    private string SphereMeshToString(MeshFilter mf, Vector3 scale) {
        ...
        for (int i = 0; i < vertices.Length; i++) {
            //球半径为0.5  修正Sphere的Center
            Vector3 vector = /*vertices[i];*/ new Vector3(vertices[i].x + -0.25f, vertices[i].y + 0.433012f, vertices[i].z);
